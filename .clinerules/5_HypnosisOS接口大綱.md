...
* **型別與定義**：`@src/催眠APP前端/models/index.ts` (包含 `AppMode`、`UserResources`、`InventoryItemState` 等)。
* **靜態常數字典**：`@src/催眠APP前端/staticData/index.ts` (包含 `HYPNOSIS_DICTIONARY`、`ITEM_DICTIONARY`、`QUEST_DICTIONARY` 等)。
* **模擬資料庫**：`@src/催眠APP前端/database/mockDatabase.ts` (包含全域模擬物件 `mockChatVariables` 與 `mockMvuVariables`)。

---

## 2. 開發期 MockAPI 接口 (記憶體模擬)
...
### 2.2 催眠功能控制
* `getMockFeatures()`: 獲取所有催眠功能 (Feature) 列表（對照 `staticData/index.ts` 中的 `HYPNOSIS_DICTIONARY`）及其購買/啟用狀態。
* `purchaseMockFeature(featureId)`: 購買特定的催眠功能。
* `updateMockFeature(featureId, data)`: 更新特定功能的啟用狀態。

### 2.3 任務與成就
* `getMockAchievements()`: 獲取成就列表與解鎖進度（對照 `staticData/index.ts` 中的 `ACHIEVEMENT_DICTIONARY`）。
* `claimMockAchievement(id)`: 領取成就獎勵。
* `getMockQuests()`: 獲取任務列表（對照 `staticData/index.ts` 中的 `QUEST_DICTIONARY`）。
...
### 3.4 AiRequestPipelineService (AI 請求管線)
