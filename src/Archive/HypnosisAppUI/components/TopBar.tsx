...
import { Battery, Bell, Settings, Monitor, Radio, Wifi, Cpu, Code, EyeOff, Ear, Fingerprint, Droplets, Wind } from 'lucide-react';
...
export const TopBar: React.FC = () => {
    ...
    // 系統功能：顯示啟用的設備 (至多 4 個)
    const enabledEquipment = installedEquipment
        .filter(eq => !disabledEquipmentIds.has(eq.id))
        .slice(0, 4);
            ...
            <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-1 border-r border-white/20 pr-3">
                    ...
                    {enabledEquipment.length === 0 && (
                        <Settings size={12} className="opacity-40" title="無啟用設備" />
                    )}
                </div>
