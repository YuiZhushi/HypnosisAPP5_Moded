...
export function useUserProfileLogic() {
  ...
  const calculateExchangeResult = (type: string | null, value: number) => {
    switch (type) {
      case 'RESTORE_ENERGY':
        return value * EXCHANGE_RATES.energyRestoreRate;
      case 'UPGRADE_MAX_ENERGY':
        return value * EXCHANGE_RATES.energyMaxUpgradeRate;
      case 'PT_TO_MONEY':
        return value * EXCHANGE_RATES.ptToMoneyRate;
      case 'MONEY_TO_PT':
        return value * EXCHANGE_RATES.moneyToPtRate;
      default:
        return 0;
    }
  };
