...
import { Settings2, PlusCircle, Trash2, Edit3, ShoppingCart, Package, Layers, Plus, X, AlertTriangle, Check } from 'lucide-react';
...
export const HypnosisManageTab: React.FC = () => {
    ...
    const { getGroupedFeatures, purchaseFeature, canPurchaseFeature, checkVipRequirement, calculateCustomCost, canCreateCustomHypnosis, createCustomHypnosis, userData } = useHypnosisManageLogic();
                    ...
                    <div className="animate-fade-in space-y-3">
                        <section className="bg-black/40 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                            ...
                            <div className="space-y-4">
                                {Object.keys(ownedGroups).sort().map(tier => (
                                    <div key={tier}>
                                        <h4 className="text-purple-300 font-bold mb-2 text-xs border-b border-purple-500/30 pb-1">{tier}</h4>
                                            ...
                                            {ownedGroups[tier].map(feature => (
                                                <div key={feature.id} className="bg-white/5 border border-white/10 rounded-lg p-3">
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        <span className={`text-[10px] px-2 py-0.5 rounded border ${feature.isPermanent ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-blue-500/20 text-blue-400 border-blue-500/30'}`}>
                                    ...
                                    <div key={combo.id} className="bg-white/5 border border-white/10 rounded-lg p-3">
                                        <div className="flex justify-between items-center mb-2">
                                            ...
                                            <div className="flex space-x-2">
                                                <button className="text-gray-400 hover:text-white transition-colors"><Edit3 size={14} /></button>
                ...
                {activeSubTab === 'SHOP' && (
                    <div className="animate-fade-in space-y-3">
                        <section className="bg-black/40 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                            ...
                            <div className="space-y-4">
                                {Object.keys(shopGroups).sort().map(tier => (
                                    <div key={tier}>
                                        <h4 className="text-purple-300 font-bold mb-2 text-xs border-b border-purple-500/30 pb-1">{tier}</h4>
                                        <div className="space-y-2">
                                            {shopGroups[tier].map(feature => {
                                                ...
                                                return (
                                                    <div key={feature.id} className="bg-white/5 border border-purple-500/30 rounded-lg p-3 hover:bg-white/10 transition-colors group">
                                                        <div className="flex justify-between items-start">
                                                            <div>
                                                                <h4 className="text-white font-bold text-sm group-hover:text-purple-300 transition-colors">{feature.name}</h4>
                                                                <span className="text-xs text-purple-400">{feature.tier}</span>
                                                            </div>
                                                            <div className="flex flex-col items-end">
                                                                <span className={`text-sm font-bold mb-1 ${canBuy ? 'text-yellow-400' : 'text-red-400'}`}>
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`text-xs px-2 py-1 rounded shadow transition-colors ${
                                                                        canBuy ? 'bg-purple-600 hover:bg-purple-500 text-white' : 'bg-gray-700 text-gray-400 cursor-not-allowed'
                                                                    }`}
                                                                >
                                                                    購買
                                                                </button>
                            ...
                            {!canCreateCustom ? (
                                <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-3 mb-4">
                                    <p className="text-xs text-purple-200">
                                        <span className="font-bold text-yellow-400">提示：</span> 只有 VIP 4 及以上等級可以製作自訂催眠。目前的等級為 {userData.vipLevel}。
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            <button
                                                ...
                                                className={`border rounded-lg p-2 text-xs transition-colors ${
                                                    customTier === 'VIP 0' ? 'bg-purple-600/30 border-purple-500/50 text-purple-200' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'
                                                }`}
                                            >微弱 (VIP 0)</button>
                                            <button
                                                onClick={() => setCustomTier('VIP 2')}
                                                className={`border rounded-lg p-2 text-xs transition-colors ${
                                                    customTier === 'VIP 2' ? 'bg-purple-600/30 border-purple-500/50 text-purple-200' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'
                                                }`}
                                            >中等 (VIP 2)</button>
                                            <button
                                                onClick={() => setCustomTier('VIP 4')}
                                                className={`border rounded-lg p-2 text-xs transition-colors ${
                                                    customTier === 'VIP 4' ? 'bg-purple-600/30 border-purple-500/50 text-purple-200' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'
                                                }`}
                                            >強烈 (VIP 4)</button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between bg-white/5 p-3 rounded-lg border border-white/10">
                                        ...
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" className="sr-only peer" checked={customPermanent} onChange={e => setCustomPermanent(e.target.checked)} />
                                    ...
                                    <button
                                        ...
                                        className={`w-full font-bold py-3 rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all flex items-center justify-center mt-4 ${
                                            customName.trim() && customDesc.trim() ? 'bg-purple-600 hover:bg-purple-500 text-white active:scale-95' : 'bg-gray-700 text-gray-500 cursor-not-allowed'
                                        }`}
                                    >
                ...
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-fade-in">
                    <div className="bg-black/60 backdrop-blur-xl border border-white/10 w-full max-w-sm rounded-2xl shadow-2xl flex flex-col animate-slide-up overflow-hidden">
                        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
                            ...
                            <button onClick={() => setShowCreateModal(false)} className="text-gray-400 hover:text-white p-1 transition-colors">
                                <X size={20} />
                            </button>
                        </div>
                        <div className="p-4 space-y-4">
                            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                                ...
                                <div className="flex space-x-2">
                                    ...
                                    <span className={`text-[10px] px-2 py-0.5 rounded border ${customPermanent ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-blue-500/20 text-blue-400 border-blue-500/30'}`}>
                            ...
                            <div className="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/10">
                                ...
                                <span className={`text-lg font-bold ${canAffordCustom ? 'text-yellow-400' : 'text-red-400'}`}>¥ {customCost.toLocaleString()}</span>
                            </div>
                            {!canAffordCustom && (
                        ...
                        <div className="p-4 border-t border-white/10 bg-black/40 flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 rounded-xl font-bold shadow-lg transition-all flex items-center justify-center ${
                                    canAffordCustom ? 'bg-purple-600 hover:bg-purple-500 text-white active:scale-95' : 'bg-gray-700 text-gray-500 cursor-not-allowed'
                                }`}
                            >
