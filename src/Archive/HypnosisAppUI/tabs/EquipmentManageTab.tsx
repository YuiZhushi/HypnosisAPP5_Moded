                    ...
                    <div className="space-y-3">
                        {installedEquipment.length === 0 ? (
                            ...
                            installedEquipment.map(eq => {
                                ...
                                return (
                                    <div key={eq.id} className={`p-3 rounded-lg border transition-colors ${isDisabled ? 'bg-white/5 border-white/5 opacity-60' : 'bg-white/10 border-white/10'}`}>
                                        <div className="flex justify-between items-start mb-2">
                                            <h4 className={`text-sm font-bold ${isDisabled ? 'text-gray-400' : 'text-white'}`}>{eq.name}</h4>
                                            <span className={`text-[10px] px-2 py-0.5 rounded ${eq.isBuiltIn ? 'bg-white/10 text-green-400' : 'bg-white/10 text-yellow-400'}`}>
                                                {eq.tier}
                                            </span>
                                        </div>
                        ...
                        {availableEquipment.length === 0 ? (
                            ...
                            availableEquipment.map(eq => {
                                ...
                                <div key={eq.id} className="bg-white/5 p-3 rounded-lg border border-white/5">
                                    ...
                                    <div className="flex justify-between items-center border-t border-white/10 pt-2">
                                        <span className={`text-xs font-bold ${canBuy ? 'text-green-400' : 'text-red-400'}`}>價格: ¥ {eq.price.toLocaleString()}</span>
                                        <button
                                            onClick={() => handlePurchaseClick(eq)}
                                            disabled={!canBuy}
                                            className={`text-xs px-3 py-1 rounded flex items-center transition-colors shadow ${
                                                canBuy ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-gray-700 text-gray-400 cursor-not-allowed'
                                            }`}
                                        >
                ...
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-fade-in">
                    <div className="bg-black/60 backdrop-blur-xl border border-white/10 w-full max-w-sm rounded-2xl shadow-2xl flex flex-col animate-slide-up overflow-hidden">
                        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
                            ...
                            <button onClick={() => setShowPurchaseModal(false)} className="text-gray-400 hover:text-white p-1 transition-colors">
                                <X size={20} />
                            </button>
                        </div>
                        <div className="p-4 space-y-4">
                            ...
                            <div className="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/10">
                                ...
                                <span className="text-lg font-bold text-yellow-400">¥ {purchasingEquipment.price.toLocaleString()}</span>
                            </div>
