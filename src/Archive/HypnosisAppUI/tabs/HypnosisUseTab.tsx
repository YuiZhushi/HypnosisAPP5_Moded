...
export const HypnosisUseTab: React.FC = () => {
    ...
    return (
      <div className="bg-black/50 p-3 rounded-b-lg border-x border-b border-purple-500/30 space-y-3 animate-slide-down -mt-px">
        ...
        <div>
          ...
          <textarea
            rows={2}
            placeholder="輸入詳細指令或設定..."
            className="w-full bg-purple-900/20 border border-purple-500/30 rounded px-2 py-1 text-white text-xs focus:outline-none focus:border-purple-400 resize-none"
          ></textarea>
        </div>
      </div>
    );
  ...
  const renderFeaturesForActiveTab = () => {
    ...
    if (tierFeatures.length === 0) {
      return <div className="text-center py-8 text-gray-500 text-sm">此等級目前沒有可用的催眠功能</div>;
    }
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-400 flex justify-between items-center pr-4">
                    <span>
                      {feature.isPermanent
                        ? `單次消耗: ${feature.costPerMin} MC`
                        : `消耗: ${feature.costPerMin} MC / 分鐘`}
                    </span>
                ...
                <div className="flex items-center gap-3">
                  {/* Toggle Switch */}
                  <div
                    className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${isEnabled ? 'bg-pink-500' : 'bg-gray-700'}`}
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all duration-200 shadow-sm ${isEnabled ? 'left-5.5 right-0.5' : 'left-0.5'}`}
                    ></div>
                  </div>
                </div>
              </div>
        ...
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-fade-in">
          <div className="bg-black/60 backdrop-blur-xl border border-white/10 w-full max-w-sm rounded-2xl shadow-2xl flex flex-col max-h-[80vh] animate-slide-up overflow-hidden">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
              ...
              <button
                onClick={() => setShowSavedCombosModal(false)}
                ...
                {Array.from(enabledFeatures).map(id => {
                  ...
                  return (
                    <div key={id} className="bg-white/5 border border-white/10 p-3 rounded-lg">
                      <div className="font-bold text-white text-sm mb-2 border-b border-white/10 pb-1">
                        {feature.name}
                      </div>
                      <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs">
                        <div className="text-gray-400">
                          持續時間:{' '}
                          <span className="text-gray-200">{feature.isPermanent ? '永久' : `${duration} 分鐘`}</span>
                        </div>
                        <div className="text-gray-400">
                          施加方式: <span className="text-gray-200">{method}</span>
                        </div>
                        <div className="col-span-2 text-gray-400">
                          施加對象:{' '}
                          <span className="text-gray-200">
                            {[...(featureSelectedTargets[id] || []), featureCustomTargets[id]]
                              .filter(Boolean)
                              .join(', ') || '未設定'}
                          </span>
                        </div>
                        <div className="col-span-2 text-gray-400">
                          備註: <span className="text-gray-200">無</span>
                        </div>
                        {missing.length > 0 && (
                          <div className="col-span-2 text-yellow-500 mt-1 flex items-center">
                            <AlertTriangle size={10} className="mr-1" /> 缺少設備: {missing.join(', ')}
                          </div>
                        )}
            ...
            <div className="p-4 border-t border-white/10 bg-black/40 flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-bold shadow-lg transition-all flex items-center justify-center ${
                  canStartHypnosis() && allMissingEquip.length === 0
        ...
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col animate-fade-in">
          <div className="bg-black/60 backdrop-blur-xl w-full h-full flex flex-col overflow-hidden animate-slide-up">
            ...
            <div className="p-4 overflow-y-auto flex-1 hypno-scrollbar">
              ...
              <div className="space-y-3 mb-6">
                {enabledFeatures.size === 0 ? (
                  <div className="text-gray-500 text-xs text-center py-4">目前沒有啟用的催眠項目</div>
                ) : (
                  Array.from(enabledFeatures).map(id => {
                    const feature = ownedFeatures.find(f => f.id === id);
                    if (!feature) return null;
