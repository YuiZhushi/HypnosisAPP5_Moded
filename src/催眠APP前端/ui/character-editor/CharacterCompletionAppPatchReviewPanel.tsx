...
import { ReviewDecision } from '../../constants/types';
...
export const CharacterCompletionAppPatchReviewPanel: React.FC<CharacterCompletionAppPatchReviewPanelProps> = ({
    ...
    try {
      if (mode === 'current') {
        if (!isBehavior) {
          ...
          newProposals = buildDiffProposals(oldNodes, newNodes, activeTab);
        } else {
          let isRawEjs = false;
          ...
          if (isRawEjs) {
             ...
             for (const nb of parsedBranches) {
                ...
                if (oldBranch) {
                    const p = buildDiffProposals(oldBranch.nodes || [], nb.nodes || [], activeTab, oldBranch.branchId);
             ...
             newProposals = buildDiffProposals(oldNodes, newNodes, activeTab, activeBranchId);
          }
        }
      } else {
        ...
        for (const [secId, dataObj] of Object.entries(sectionTempData)) {
          ...
          const p = buildDiffProposals(oldNodes, newNodes, secId);
          newProposals = newProposals.concat(p);
        }
              ...
              if (secId && sectionBody.includes('<%')) {
                ...
                try {
                  ...
                  if (parsed && typeof parsed === 'object') {
                    ...
                    const p = buildDiffProposals(oldNodes, newNodes, secId, defaultBranch?.branchId);
                    newProposals = newProposals.concat(p);
                  }
                } catch (e) {
          ...
          for (const [secId, newBranches] of Object.entries(rawEjsBranchesData)) {
            ...
            for (const nb of newBranches) {
              const oldBranch = currentBranches.find(ob => ob.operator === nb.operator && ob.threshold === nb.threshold) 
                             || currentBranches.find(ob => ob.kind === 'else' && nb.kind === 'else');
              if (oldBranch) {
                const p = buildDiffProposals(oldBranch.nodes || [], nb.nodes || [], secId, oldBranch.branchId);
                newProposals = newProposals.concat(p);
              } else {
                // New branch that doesn't exist in current data — generate add proposals for all nodes
                const p = buildDiffProposals([], nb.nodes || [], secId);
                newProposals = newProposals.concat(p);
              }
            }
          }
  ...
  const handleApply = useCallback(() => {
    ...
    for (const [ks, ps] of Object.entries(bySectionBranch)) {
      ...
      if (!isBehavior) {
        const out = applyApprovedProposals(
