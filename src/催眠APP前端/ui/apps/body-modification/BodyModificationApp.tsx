...
export const BodyModificationApp: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const globalModifications = Object.values(bodyModsDict).filter(
    mod => mod.scope === 'global'
  );
  ...
  const hasInstalledMod = (slotId: string) => {
    return Object.values(npcData.ownedBodyModifications || {}).some(
      state => {
        const def = bodyModsDict[state.id];
      ...
      <div className="flex-1 flex overflow-hidden w-full">
        ...
        <div className="w-[45%] md:w-[42%] bg-[#05030d] border-r border-purple-950/30 flex flex-col overflow-y-auto hypno-scrollbar p-2 shrink-0">
          ...
          <div className="grid grid-cols-2 gap-1 mb-3 bg-[#110d29]/40 p-0.5 rounded border border-purple-950/30 text-[10px] shrink-0 font-mono">
            <button
              onClick={() => { setActiveTab('parts'); setActionError(null); setActionSuccess(null); }}
            ...
            <button
              onClick={() => { setActiveTab('global'); setActionError(null); setActionSuccess(null); }}
              className={`py-1 rounded transition-all cursor-pointer ${
                activeTab === 'global' 
                ...
                <div className="space-y-1.5">
                  {slotsByCategory.cranial.map(slot => {
                    ...
                    return (
                      <button
                        key={slot.id}
                        onClick={() => { setSelectedSlot(slot.id); setActionError(null); setActionSuccess(null); }}
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center justify-between w-full text-[9px] text-gray-500">
                          <span>敏感度:</span>
                          <span style={{ color: MockApi.getGradeColor(MockApi.getStatGrade(sensitivity, -100, 100)) }} className="font-bold">
            ...
            <>
              ...
              <div className="p-3 bg-[#0d0922] border-b border-purple-900/30 shrink-0 font-mono">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[8px] uppercase text-purple-400 font-bold block">// SELECTED SLOT ACTIVE</span>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs font-bold" style={{ color: MockApi.getGradeColor(MockApi.getStatGrade(activeSlotData?.sensitivity ?? 0, -100, 100)) }}>
                      {activeSlotData?.sensitivity ?? 0} ({MockApi.getStatGrade(activeSlotData?.sensitivity ?? 0, -100, 100)})
                    </span>
                  </div>
                </div>

                {/* 自訂部位額外屬性 (鬆緊、熟練) */}
                {activeSlotData && (activeSlotData.tightness !== undefined || activeSlotData.proficiency !== undefined) && (
                          ...
                          >
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className={`text-[8px] px-1 rounded font-mono ${
                                    def.category === 'material' ? 'bg-cyan-950 text-cyan-400 border border-cyan-800/40' :
                                    def.category === 'shape' ? 'bg-pink-950/60 text-pink-400 border border-pink-800/40' :
                                    'bg-purple-950/60 text-purple-400 border border-purple-800/40'
                                  }`}>
                                    {def.category === 'material' ? '材質' : def.category === 'shape' ? '形狀' : '可疊加'}
                                  </span>
                                </div>
                            ...
                            <div className="mt-2 flex flex-wrap gap-1">
                              {def.modifiers?.map((mod, idx) => (
                                <span key={idx} className="text-[9px] bg-[#070512] text-purple-300 px-1.5 py-0.5 rounded font-mono">
                                  {mod.targetType === 'global_stat' ? '全域' : '部位'} {mod.statName} {mod.operator}{mod.value}
                                </span>
                              ))}
                            </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {localModifications.map(def => {
                      ...
                      const canPerform = isConditionsMet && isItemsSufficient && isMoneySufficient && isPtsSufficient && isEnergySufficient && !isInstalled;
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <button
                              ...
                              className={`text-[10px] px-2.5 py-1 rounded font-semibold cursor-pointer shrink-0 transition-all font-mono ${
                                isInstalled ? 'bg-purple-950/40 text-purple-600 border border-purple-900/10 cursor-not-allowed' :
                                canPerform ? 'bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold' :
                          ...
                          <div className="mt-2 pt-2 border-t border-purple-950/20 grid grid-cols-2 gap-1 text-[9px] text-gray-500 font-mono">
                            <div>
                              手術費用: <span className={isMoneySufficient ? 'text-yellow-300' : 'text-red-400'}>
                                {def.cost.money ? formatMoney(def.cost.money) : '免費'}
                              </span>
                            </div>
                            <div>
                              MC 點數: <span className={isPtsSufficient ? 'text-purple-300' : 'text-red-400'}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <button
                          ...
                          className={`text-[10px] px-3 py-1.5 rounded font-semibold cursor-pointer shrink-0 transition-all font-mono ${
                            isInstalled ? 'bg-cyan-950/40 text-cyan-700 border border-cyan-900/10 cursor-not-allowed' :
                            canPerform ? 'bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500 hover:from-cyan-600 hover:to-pink-600 text-white font-bold shadow-[0_0_8px_rgba(6,182,212,0.15)]' :
                            'bg-gray-800 text-gray-500 border border-gray-700 cursor-not-allowed'
                          }`}
                        >
                      ...
                      <div className="mt-3 pt-2.5 border-t border-cyan-950/20 grid grid-cols-2 gap-1 text-[9px] text-gray-500 font-mono">
                        <div>
                          重組費用: <span className={isMoneySufficient ? 'text-yellow-300' : 'text-red-400'}>
                            {def.cost.money ? formatMoney(def.cost.money) : '免費'}
                          </span>
                        </div>
                        <div>
                          催眠點數: <span className={isPtsSufficient ? 'text-cyan-300' : 'text-red-400'}>
                            {def.cost.pts ? `${def.cost.pts} PT` : '無'}
                          </span>
                        </div>
                      </div>
