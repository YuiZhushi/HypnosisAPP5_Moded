...
import { CharacterMockApi } from '../../../shared/api/characterMockApi';
import { CharacterBackgroundData, PromptTemplate, EJSnode } from '../../../database/characterMockData';
import {
  Users,
  Edit3,
...
const SettingsView: React.FC = () => {
  ...
  useEffect(() => {
    const loadPrompts = async () => {
      setLoading(true);
      try {
        const data = await CharacterMockApi.getPrompts();
        setPrompts(data);
      } finally {
        setLoading(false);
      }
...
export const CharacterBackgroundApp: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const data = await CharacterMockApi.getCharacters();
        setCharacters(data);
      } catch (e) {
  ...
  const handleCreateCharacter = async (name: string) => {
    ...
    try {
      const saved = await CharacterMockApi.saveCharacter(newChar);
