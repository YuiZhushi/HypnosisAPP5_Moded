...
import { ApiSettings } from '../../constants/interfaces';
import type { SettingsPromptTuningConfig } from '../../constants/settings/settingsPromptDefaults';
import * as AiRequestPipelineService from '../../shared/llm/aiRequestPipeline';
import {
  getApiSettings,
  updateApiSettings,
  fetchAvailableModels,
  getSettingsPromptConfig,
  getDefaultSettingsPromptConfig,
  updateSettingsPromptConfig,
...
function ApiSettingsSection({ onDirty }: SettingsSectionProps) {
  ...
  useEffect(() => {
    ...
    if (saved) setForm({ ...DEFAULT_API, ...saved } as ApiSettings);
  }, []);
  ...
  const handleFetchModels = async () => {
    ...
    try {
      const list = await fetchAvailableModels(form.apiEndpoint, form.apiKey ?? '');
      setModels(list);
...
function SettingsPromptTuningSection({ onDirty }: SettingsSectionProps) {
  ...
  useEffect(() => {
    const loaded = getSettingsPromptConfig();
    setSettings(loaded);
  }, []);
  ...
  const handleRestoreDefault = () => {
    setSettings(getDefaultSettingsPromptConfig());
  ...
  const handleSave = async () => {
    ...
    setSaveStatus('saving');
    try {
      await updateSettingsPromptConfig(settings);
      setSaveStatus('saved');
