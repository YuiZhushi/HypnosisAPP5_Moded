...
import { z } from 'zod';
import type { QuestStatus, StreamMode, EditorPromptModuleType, SubscriptionTier, VipTier } from '../types';
import type { CustomHypnosisDef, EditorPromptModule } from '../interfaces';
...
export type CalendarCrudOp =
  | { opId: string; type: 'add'; eventId: string; month: number; day: number; title: string; description?: string; createdAt: number }
...
export type PersistedStore = {
  ...
  editorPromptModules?: Record<string, {
    id: string;
    title: string;
    content: string;
...
export const STORE_SCHEMA: z.ZodType<PersistedStore> = z
  .object({
    ...
    subscription: z
      ...
      .optional(),
    features: z
      .record(z.string(), z.object({ isEnabled: z.boolean().optional(), userNote: z.string().optional(), userNumber: z.coerce.number().optional() }).passthrough())
      .default({}),
    ...
    customQuests: z
      .record(z.string(), z.object({ name: z.string(), condition: z.string(), rewardMcPoints: z.coerce.number(), createdAt: z.coerce.number() }))
      .default({}),
    calendarEvents: z
      .record(z.string(), z.object({ id: z.string(), month: z.coerce.number(), day: z.coerce.number(), title: z.string(), description: z.string().optional() }))
      .default({}),
    calendarCRUD: z
      .object({
        version: z.coerce.number().default(1),
        ...
        bridge: z
          .object({
            deleteFloor: z.object({ triggered: z.coerce.boolean().default(false), deleteFrom: z.coerce.number().optional() }).default({ triggered: false }),
            deleteSwipe: z.object({ triggered: z.coerce.boolean().default(false), floor: z.coerce.number().optional(), swipeId: z.coerce.number().optional(), newSwipeId: z.coerce.number().optional() }).default({ triggered: false }),
            switchSwipe: z.object({ triggered: z.coerce.boolean().default(false), floor: z.coerce.number().optional() }).default({ triggered: false }),
          })
          .default({ deleteFloor: { triggered: false }, deleteSwipe: { triggered: false }, switchSwipe: { triggered: false } }),
      })
      .optional(),
    customHypnosis: z
      .record(z.string(), z.object({
        ...
        costType: z.enum(['ONE_TIME', 'PER_MINUTE']), costValue: z.coerce.number(),
        notePlaceholder: z.string().optional(), createdAt: z.coerce.number(), researchCost: z.coerce.number(),
      }))
      .default({}),
    apiSettings: z
      .object({
        apiKey: z.string().default(''), apiEndpoint: z.string().default(''), modelName: z.string().default(''),
        temperature: z.coerce.number().min(0).max(2).default(0.7), maxTokens: z.coerce.number().int().min(1).default(8192),
        topP: z.coerce.number().min(0).max(1).default(1),
        presencePenalty: z.coerce.number().min(-2).max(2).default(0.2), frequencyPenalty: z.coerce.number().min(-2).max(2).default(0.15),
        streamMode: z.enum(['streaming', 'fake_streaming', 'non_streaming']).default('non_streaming'),
      })
      .optional(),
    settingsPromptTuning: z
      .object({
        modules: z.record(z.string(), z.object({ id: z.string(), title: z.string(), content: z.string(), enabled: z.coerce.boolean().default(true) })).default({}),
        moduleOrder: z.array(z.string()).default([]),
        placeholders: z.record(z.string(), z.object({
          key: z.string(), value: z.string().default(''), enabled: z.coerce.boolean().default(true),
          source: z.enum(['built_in', 'user', 'worldbook', 'runtime']).default('user'),
          resolverType: z.enum(['static', 'function']).default('static'), scope: z.literal('app').default('app'),
        })).default({}),
      })
      .optional(),
    editorPromptModules: z
      .record(z.string(), z.object({
        ...
        sectionId: z.string().optional(), order: z.coerce.number().default(99),
      }))
      .optional(),
  })
  .default({
    version: 1, debugEnabled: false, hasUsedHypnosis: false,
    features: {}, purchases: {}, achievements: {}, quests: {},
    customQuests: {}, calendarEvents: {}, calendarCRUD: DEFAULT_CALENDAR_CRUD, customHypnosis: {},
  });
