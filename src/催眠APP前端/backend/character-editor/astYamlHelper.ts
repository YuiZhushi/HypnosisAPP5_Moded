/**
 * Character Editor APP 後端 — YAML/AST 轉換工具
 *
 * 從 astYamlHelper.ts 遷移。
 * 提供 EditorNode[] ↔ YAML 雙向轉換。
 */
  ...
  if (typeof obj === 'object') {
    return Object.entries(obj as Record<string, unknown>).map(([key, val]) => {
      ...
      if (Array.isArray(val)) {
        return { id: nextId(), key, type: 'list' as NodeType, value: '', children: yamlToTree(val), isLocked };
      }
      if (typeof val === 'object' && val !== null) {
        return { id: nextId(), key, type: 'object' as NodeType, value: '', children: yamlToTree(val), isLocked };
      }
      return { id: nextId(), key, type: 'string' as NodeType, value: String(val ?? ''), children: [], isLocked };
    });
  }
...
function nodeToYamlValue(node: EditorNode): unknown {
  ...
  if (node.type === 'list') return node.children.map(nodeToYamlValue);
...
export function treeToYaml(nodes: EditorNode[]): unknown {
  ...
  if (rootLooksLikeArray) return nodes.map(nodeToYamlValue);
