...
1. **Runtime 變數**：僅存在於 iframe 記憶體中（如 React 組件的 `useState`），重新整理頁面即消失，無法持久化。
...
### 2.1 開發期 (Mock 變數)
* **資料來源**：變數系統目前由 `database/mockDatabase.ts` 內的全域模擬物件 `mockChatVariables` 與 `mockMvuVariables` 在記憶體中模擬維護。
...
* **Zod 驗證與糾錯**：所有寫入酒館變數的資料，必須通過定義在 `models/index.ts` 中的 Zod Schema 驗證與糾錯（如 `z.coerce.number()`），自動補齊舊存檔缺失的欄位並填入預設值。
...
### 3.1 用戶資源 (`UserResources` 型別，定義於 `models/index.ts`) -> 聊天變數 `系统`
* `mcEnergy` -> `系统._MC能量`
...
* **寫入規則**：所有對 store 的寫入必須透過後端 API / MockAPI 完成，寫入後自動調用 `MvuBridge.syncPersistedStore(...)` 做 MVU 同步。
...
* 用於在變數更新命令實際執行前，攔截並修復 AI 輸出的格式錯誤。
...
* **處理流程**：在收到 AI 回傳的 raw 內容時，需手動調用 `Mvu.parseMessage` 解析該內容，再透過 `MvuBridge` 將更新後的變數寫回當前樓層。
