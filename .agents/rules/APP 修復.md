...
description: 專門用於分析、診斷並修復 HypnosisOS 前端 APP 的錯誤與調整實作機制的開發輔助技巧。
---
...
## 核心開發原則
1. **先觀察，後修改**：不要盲目覆寫檔案。先使用閱讀工具（`view_file`, `grep_search`）全面理解原有代碼，尤其是組件與 Service 間的互動邏輯。
...
4. **驗證與診斷**：善用 `chrome-devtools` 的開發者工具 Console，並對致命錯誤加上拋出例外或以 `console.warn / error` 記錄。
...
3. **精準修復**：使用 `multi_replace_file_content`，確保不破壞前後的其他代碼。
4. **清理除錯痕跡**：移除除錯用的 `console.log`，只保留關鍵的 `console.info`。
